...
import { Lead, LeadStatus, LeadStage } from "./entities/lead.entity";
  ...
  async createManualLead(dto: CreateLeadDto, userId: number): Promise<any> {
    try {
      ...
      const lead = this.leadRepository.create({
        ...
        lead_source: dto.lead_source as any, // Cast to LeadSource enum
        medium: dto.medium,
        lead_source_id: dto.source_id,
        lead_medium_id: dto.medium_id,
        ...
        note: dto.note || null,
        lead_status_id: 1, // Default as per URS
  ...
  async getLeadList(userId: number, role: string, filters: any) {
    try {
      ...
      let query = this.leadRepository
        .createQueryBuilder("leads")
        .leftJoin(User, "owner_user", "owner_user.id = lead.fk_owner_id")
        .select([
          "leads.id as id",
      ...
      if (role === UserRole.AGENT) {
        ...
        query = query.andWhere(
          "(owner_agent.fk_manager_id = :managerId OR lead.fk_owner_id = :ownerId)",
